...
1. 区分逻辑世界尺寸与显示视窗尺寸：
   - 合作逻辑世界：3200x1920。
   - 合作 camera viewport：1600x960。
   - 当前对战模式尺寸保持现状，除非后续统一为 1600x960。
...
4. camera 只显示以本地玩家为中心的 1600x960 视窗：
   - 玩家在地图中央区域时 camera 跟随玩家中心。
   - 玩家贴墙时 camera clamp 到世界边界，不再强行让玩家处于屏幕中心。
...
4. 准心 HUD：
   - 现有 `p:xx` 下方增加 money 数量。
   - p 和 money 都按玩家独立显示。
5. 新增合作计分常量目录，建议 `packages/constants/src/battle/collaborate-scoring.ts`：
   - 击败 minion/elite/boss 分数。
   - 拾取 point/money 分数。
   - 擦弹分数。
6. 右上角显示当前本地玩家得分，必要时也显示队友得分。
...
- 合作测试竞技场为 3200x1920 世界，camera 为本地玩家中心视窗并正确 clamp。
- 玩家之间子弹不互相伤害。
- Neutral 弹幕能伤害玩家。
