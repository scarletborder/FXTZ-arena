...
export class BattleStartScene extends Phaser.Scene {
  ...
  private showCreateRoomForm(): void {
    ...
    c.add(createFightButton(this, cx - 80, py + ph - 60, 140, 44, "创建", () => {
      connectionManager.send({ type: "create_room", name: roomName, password: roomPassword || undefined, mapId: "arena_standard", lifeCount: parseInt(lifeLabel.text, 10), costLimit: 10 });
      c.destroy();
      this.formContainer = null;
      this.activeField = null;
