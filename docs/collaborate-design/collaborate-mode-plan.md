...
3. 左上角 spellCard HUD：
   - 每个 elite/boss 一行。
   - 显示“怪物名:星星”。这里的星星是icon
   - 星星数量等于该怪物剩余符卡数量，每剩余1张符卡未开始释放，那么就有1个星星。例如，在释放第一非符(刚开打)时，数额等于该怪物拥有的符卡数量，在释放最后一张符卡时(打完这张符卡就结束)，数额为0。
   - 当前符卡进行中时，在右侧显示剩余秒数。
4. spellCard 状态机：
   - 非符 1：血量从满降到蓝色阈值。
   ...
   - 最后一步始终为符卡。
   - 最后符卡打空或时间到后，怪物正式阵亡。
5. 每个 elite/boss 独立推进自己的 spellCard，不共享计时或血量。
...
16. 商店中不应该出现本名玩家已经拥有的能力卡片，如果实在抽不到了，但是依然要出现该稀有度商品，那么显示缺货。两名玩家看到的商店可以不一样
