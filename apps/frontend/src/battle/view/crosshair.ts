...
export class CrosshairView {
  ...
  constructor(scene: Phaser.Scene) {
    this.crosshair = scene.add.image(640, 360, "cursor").setOrigin(0.5).setScale(0.22).setDepth(Depth.Crosshair);
    this.ammoBack = scene.add.rectangle(0, 0, 10, 58, 0x111820, 0.9).setOrigin(0.5, 0).setDepth(Depth.Crosshair);
    this.ammoFill = scene.add.rectangle(0, 0, 8, 56, 0x8b99aa, 1).setOrigin(0.5, 0).setDepth(Depth.CrosshairFill);
    this.ammoOutline = scene.add.rectangle(0, 0, 10, 58, 0x000000, 0).setOrigin(0.5, 0).setStrokeStyle(2, 0xd7e3ef, 0.9).setDepth(Depth.CrosshairText);
    this.ammoText = scene.add.text(0, 0, "", {
      fontFamily: "Arial, 'Microsoft YaHei', sans-serif",
      fontSize: "13px",
      color: "#d7e3ef",
    }).setOrigin(0.5).setDepth(Depth.CrosshairText);
    ...
    this.bombIcons = Array.from({ length: 6 }, () =>
      scene.add.image(0, 0, "bomb").setOrigin(0.5).setScale(0.085).setTint(0xaec7ff).setDepth(Depth.Crosshair),
    );
  }
