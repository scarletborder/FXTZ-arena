...
5. `battle/manager` 是迁移源目录。完成一个纵向切片就删除对应旧文件，不创建转发文件。
...
| `battle/runtime-adapter.ts`          | `battle/adapters/phaser/runtime-adapter.ts`                                    | 已完成                      |
| `battle/manager/rollback-manager.ts` | `battle/session/rollback-history.ts`                                           | 与 BattleSession 合并时进行 |
| `battle/manager/replay-manager.ts`   | `battle/session/replay-session.ts`                                             | 先分离 Phaser 场景转场      |
| `battle/manager/debug-manager.ts`    | `battle/adapters/phaser/debug-controller.ts`                                   | 先提取纯调试命令接口        |
| `battle/view/controller/*`           | `battle/adapters/phaser/*` 或 `battle/view/*`                                  | 按是否只负责渲染判断        |

不进行一次性全目录搬迁。每次移动必须伴随接口收敛、调用者迁移、旧文件删除和测试，避免只改变路径而不改善耦合。
