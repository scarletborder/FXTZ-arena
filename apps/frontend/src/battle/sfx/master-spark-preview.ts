...
export function renderMasterSparkPreviewSfx(graphics: Phaser.GameObjects.Graphics, params: MasterSparkPreviewSfxParams): void {
  graphics.clear();
  graphics.setPosition(params.x, params.y);
  graphics.setRotation(params.angle);
  graphics.lineStyle(Math.max(2, params.width), params.color, params.alpha ?? 0.35);
  graphics.beginPath();
  graphics.moveTo(0, 0);
  graphics.lineTo(params.length, 0);
  graphics.strokePath();
  graphics.lineStyle(1, 0xffffff, 0.45);
  graphics.strokeRect(0, -params.width, params.length, params.width * 2);
}
