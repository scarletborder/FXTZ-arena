...
export class CollaborateShopPanel {
  ...
  update(model: BattleShopPresentationModel): void {
    ...
    const hoverItem = this.orderedGoods.find(
      (item) => item.id === this.hoverItemId,
    );
    ...
    this.money?.setColor(
      !hoverItem || hoverItem.kind === "sold_out"
        ? "#f7e5aa"
        : model.localMoney >= hoverItem.price
          ? "#92e6a7"
          : "#ff6b6b",
    );
    ...
    this.renderGoods(
      model.purchasedItemIds,
      this.interactionDisabled || model.localReady || model.localRevived,
    );
    this.renderPreview(hoverItem, model.localRevived);
    this.renderBagDialog(model.activeCardId);
    this.readyButtonText?.setText(
      model.localReady || model.localRevived
        ? t("battle.shop_ready_done")
        : t("battle.shop_ready"),
    );
    this.readyButtonBg?.setFillStyle(
      model.localReady || model.localRevived ? 0x50606a : 0xd94b4b,
      1,
    );
    this.readyButton?.setAlpha(
      model.localReady || model.localRevived ? 0.65 : 1,
    );
    this.bagButtonBg?.setFillStyle(
      this.bagDialog?.visible ? 0x31424c : 0x182834,
      1,
    );
  ...
  private syncGoods(goods: readonly CollaborateShopItemState[]): void {
    ...
    this.orderedGoods.splice(
      0,
      this.orderedGoods.length,
      ...baseGoods,
      ...cardGoods,
    );
  ...
  private renderGoods(
    purchasedItemIds: readonly string[],
    disabled: boolean,
  ): void {
    const baseCount = this.orderedGoods.filter((item) =>
      BASE_ITEM_KINDS.has(item.kind),
    ).length;
    ...
    for (let index = 0; index < this.itemVisuals.length; index += 1) {
      ...
      const startX =
        SHOP_GOODS_CENTER_X - ((rowCount - 1) * SHOP_ITEM_STEP) / 2;
      const bought = purchasedItemIds.includes(item.id);
      const soldOut = item.kind === "sold_out";
      const selected =
        this.keyboardSurface === "goods" && index === this.selectedItemIndex;

      visual.itemId = item.id;
      visual.container.setPosition(
        startX + rowIndex * SHOP_ITEM_STEP,
        row === 0 ? -96 : 54,
      );
      ...
      visual.soldOutText
        .setText(t("battle.shop_item_sold_out"))
        .setVisible(bought);
  ...
  private ensureItemVisuals(count: number): void {
    ...
    while (this.itemVisuals.length < count) {
      ...
      container.on("pointerover", () => {
        const visual = this.itemVisuals.find(
      ...
      container.on("pointerdown", () => {
        ...
        if (visual?.itemId && !this.interactionDisabled)
          this.callbacks.onPurchase(visual.itemId);
      });
  ...
  private renderPreview(
    item: CollaborateShopItemState | undefined,
    revived: boolean,
  ): void {
    this.ensurePreview();
    if (revived) {
    ...
    const text = itemPreview(
      item,
      getKeyDisplayName(settingsRepository.get().keybinds.activeCard),
    );
  ...
  private renderBagDialog(activeCardId: string | undefined): void {
    ...
    for (let index = 0; index < this.bagCardVisuals.length; index += 1) {
      ...
      const selected =
        this.keyboardSurface === "bag" && index === this.selectedBagCardIndex;
      visual.container.setVisible(true);
      visual.container.setPosition(-180 + index * 120, 18);
      visual.bg.setFillStyle(
        selected ? 0x2f3f24 : active ? 0x274634 : 0x182834,
        1,
      );
      visual.bg.setStrokeStyle(
        selected ? 3 : 2,
      ...
      visual.icon
        .setTexture(abilityCardIconTextureKey(card.id))
        .setVisible(true);
  ...
  private ensureBagCardVisuals(count: number): void {
    ...
    while (this.bagCardVisuals.length < count) {
      ...
      container.on("pointerdown", () => {
        const visual = this.bagCardVisuals.find(
          (item) => item.container === container,
        );
  ...
  private moveGoodsSelection(dx: -1 | 0 | 1, dy: -1 | 0 | 1): void {
    ...
    if (dy !== 0) {
      this.selectedItemIndex = moveGoodsRow(
        this.orderedGoods,
        this.selectedItemIndex,
        dy,
      );
    } else if (dx !== 0) {
      this.selectedItemIndex = wrapIndex(
        this.selectedItemIndex + dx,
        this.orderedGoods.length,
      );
    }
    this.hoverItemId = this.orderedGoods[this.selectedItemIndex]?.id;
  ...
  private moveBagSelection(dx: -1 | 0 | 1, dy: -1 | 0 | 1): void {
    ...
    if (delta !== 0) {
      this.selectedBagCardIndex = wrapIndex(
        this.selectedBagCardIndex + delta,
...
function moveGoodsRow(
  ...
  const baseCount = goods.filter((item) =>
    BASE_ITEM_KINDS.has(item.kind),
  ).length;
  ...
  const targetIndex = Math.min(
    targetCount - 1,
    Math.round(
      (sourceIndex / Math.max(1, sourceCount - 1)) *
...
function checkText(player: "Player1" | "Player2", checked: boolean): string {
  return t(
    checked ? "battle.shop_checked_player" : "battle.shop_unchecked_player",
    {
...
function setItemIcon(
  ...
  if (item.kind === "ability_card" && item.abilityCardId) {
    visual.iconImage
      .setTexture(
        abilityCardIconTextureKey(item.abilityCardId as AbilityCardId),
      )
      .setVisible(true);
...
function itemPreview(
  ...
  if (card?.kind === "active") {
    lines.push(
      ...
      t("battle.shop_preview_uses", {
        uses:
          card.useLimit === "infinite"
            ? t("battle.shop_uses_infinite")
            : card.useLimit,
      }),
    );
  }
  ...
  lines.push(
    ...
    "",
    t("battle.shop_buy_hint", { key: activeKeyName }),
  );
  return lines.join("\n");
}
...
function formatMoneyDisplay(params: {
  readonly model: BattleShopPresentationModel;
